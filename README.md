...
### Data Preprocessing 
Use pandas to do the data cleaning and generate the dataframe for log data.

### Feature 1
Group the data by address and count the appearing times and take the top 10 write to file.

### Feature 2
Group the data by requests and sum all the data rows with the same request type then take the top 10 write to the output file.

### Feature 3
Use two pointer to go through the time list and update the record of busiest hour. Time complexity is O(n).

### Feature 4
Designed the O(n) algorithms with two pointers to record the logs that should have been blocked.
